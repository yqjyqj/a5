...
public abstract class MinMaxAI extends Controller {
	...
	protected @Override Location nextMove(Game g) {
		while(estimate(g.getBoard())!=Integer.MIN_VALUE){//while game is not lost yet
			for(Location loc: moves(g.getBoard())){
				System.out.println(estimate(g.getBoard()));
				if(estimate(g.getBoard())==Integer.MAX_VALUE) return loc;
				else{
						Board newBoard=g.getBoard().update(me, loc);
						nextMove(g);
